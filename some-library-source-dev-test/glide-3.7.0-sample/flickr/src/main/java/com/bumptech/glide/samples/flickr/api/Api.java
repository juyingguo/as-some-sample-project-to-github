...
public class Api {
     ...
     * @param text
     */
    public void searchCustomResult(final String text) {
        if (lastSearchResult != null && TextUtils.equals(lastSearchResult.searchString, text)) {
            for (SearchListener listener : searchListeners) {
                listener.onSearchCompleted(lastSearchResult.searchString, lastSearchResult.results);
            }
...
//            JSONArray photos = searchResults.getJSONObject("photos").getJSONArray("photo");
            ...
            String[] URLS = Data.URLS_CUSTOM;
            for (int i=0;i<URLS.length;i++){
                JSONObject jsonObject = new JSONObject();

                jsonObject.put("id","i" + i);
                jsonObject.put("owner","i" + i);
                jsonObject.put("title","i" + i);
                jsonObject.put("server",URLS[i]);//server 作为url
                jsonObject.put("farm","i" + i);
                jsonObject.put("secret","i" + i);
